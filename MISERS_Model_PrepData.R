...
get_scale <- function(
  ...
  SS.all <- expand.grid(start.year = (1 - (max.age - min.age)):nyear, ea = range_ea, age = min.age:(r.max - 1)) %>% 
    ...
    mutate(year = year + init.year - 1,
           start.year = start.year + init.year - 1 # convert to calendar year. 
    ) %>% 
...
get_benefit.disb <- function(
  .disb, #= tailored_demoData$retirees,
  .paramlist = paramlist,
  .Global_paramlist  = Global_paramlist
){
...
# get_initPop(get_tierData(init_actives_all, Tier_select_),
#             get_tierData(init_retirees_all, Tier_select_),
#             get_tierData(init_terms_all, Tier_select_),
#             get_tierData(init_disb_all, Tier_select_)
#             )
...
get_initPop_tier <- function(Tier_select_, grouping_ =  paramlist$Grouping){
  ...
  init_actives        <- get_tierData(init_actives_all, Tier_select_)
  init_retirees       <- get_tierData(init_retirees.la_all, Tier_select_)
  init_terms          <- get_tierData(init_terms_all, Tier_select_)
  init_disb           <- get_tierData(init_disb.la_all,  Tier_select_)
  #init_beneficiaries <- get_tierData(init_beneficiaries_all, Tier_select_)
  
  get_initPop(init_actives, init_retirees,
              init_terms, init_disb)
...
# get_benefit_tier("t3")
# get_initPop_tier("t3")
# get_entrantsDist_tier("t6")


get_tierData(init_terms_all, "t1")
