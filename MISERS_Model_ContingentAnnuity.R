...
get_contingentAnnuity <- function(Tier_select_,
                                  factor.ca,
                                  range_age.ben, # age range for beneficiaries. 
  ...
  # decrement.model_  = decrement.model
  # factor.ca = 0.8
  # range_age.ben = min(paramlist$range_age):max(paramlist$range_age.r)
  # apply_reduction = T
  # paramlist_        = paramlist
  # Global_paramlist_ = Global_paramlist
  #  
...
get_liab.ben <- function(gender.R, 
                         ...
                         range_age.r_ = range_age.ben,
                         mortality    = decrement.model_,
                         reduction.factor = 1
                         ){
  # For a given range of retirement ages and gender, this function calculates the following for each retirement age:
  # 1) the individual liability and benefit payments for retirees and survivors in each year after retirement, by retirement age. (initial payment is 1)  
  # 2) number of retirees(w/ and w/o living spouses) and survivors in each year after retirement, by retirement age. (initial population is 1)
  # 3) total liability and benefit payment for retirees and survivors in each year after retirement, by retirementa age 
  ...
  # gender.R  <- "M"  # R for retirees, "S" for survivors
  # cola_ = cola
  # i_ = i
  # factor.ca_   = factor.ca  # benenficiaries' benefit as a proportion of retiree's benefit.
  # range_age_   = range_age
  # range_age.r_ = range_age.ben
  # mortality    = decrement.model_
  # reduction.factor = 1
  # 
  # reduction.factor = reduction.F
  ...
  df <- expand.grid(age = (min(range_age.post) - 3):max(range_age_), age.r = range_age.r_) %>% 
    ...
    mutate(
      ...
      # Beneficiaries are in either of the three status:
      ...
      pRxm.R     = ifelse(age == min(age), 1, lag(cumprod(pxm.R))),  # cumulative probability that retiree surives at each year, starting from the retirement age.
      ax.r.R     = get_tla(pxm.R, i_, COLA.scale),                   # actuarial present value of future benefit at each age, for a $1 payment at that age. 
      ...
      ax.b.S     = get_tla(pxm.S, i_, COLA.scale),   # cumulative probability that retiree surives at each year, starting from the retirement age.
      liab.ben.indiv = factor.ca_ * B.la * ax.b.S,   # actuarial present value of future benefit at each age, for a $1 payment at that age. 
  ...
  #df %>% filter(age.r == 20) %>% select(n.R1S0, n.R0S1, n.R0)
  #df %>% filter(age.r == 20)%>% select(n.R1S0, n.R0S1, n.R0, n.R1S1, pxm.R, pxm.S, n.newR1S0, n.newR0S1) %>% fn_demo
...
pct.F.actives <- pct.female
pct.M.actives <- pct.male
...
#*********************************************************************************************************
#              ##  Calculate actuarial present value of future benefit at each age.  ####
#*********************************************************************************************************

# df.test <- df.all[df.all$age.r == 41, c("age", "B.ca.sum.1", "liab.ca.sum.1")]
# 
# df.test %<>% mutate(DF = v^(row_number() - 1),
#                     D.B = B.ca.sum.1 * DF)
# df.test %>% head
# 
# df.test$D.B %>% sum
